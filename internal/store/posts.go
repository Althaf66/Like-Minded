...
func (s *PostStore) Create(ctx context.Context, post *Post) error {
	query := `INSERT INTO posts (content, title, user_id, tags) VALUES ($1,$2,$3,$4) 
...
func (s *PostStore) GetbyID(ctx context.Context, id int64) (*Post, error) {
	...
	err := s.db.QueryRowContext(ctx, query, id).Scan(
		...
		&post.CreatedAt, &post.UpdatedAt, pq.Array(&post.Tags), &post.Version)
...
func (s *PostStore) GetUserFeed(ctx context.Context, userid int64, fq PaginatedFeedQuery) ([]PostMetadata, error) {
	query := `
	SELECT 
	...
	for rows.Next() {
		var p PostMetadata
		err := rows.Scan(&p.ID, &p.UserID, &p.Title, &p.Content, &p.CreatedAt, &p.Version,
			pq.Array(&p.Tags), &p.User.Username, &p.CommentCount)
		if err != nil {
			return nil, err
		}
		feed = append(feed, p)
